...
:Need to enumerate each dll file (hopefully only one, but go with the last one)
for %%a in (\\%comp%\admin$\System32\Macromed\Flash\NPS*.dll) do for /F "tokens=5" %%b in ('filever "%%a"') do set File_Version=%%b
