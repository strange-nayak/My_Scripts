...
@ECHO Machine,Accessible?,File Version,new_chrome.exe,Chrome.exe running time>%ResultsFolder%\Header.txt
...
SET FILE1=\\%comp%\C$\Program Files (x86)\Google\Chrome\Application\chrome.exe
SET FILE2=%FILE1: (x86)=%
if exist "%FILE1%" (
...
@SET File_Version=Not Found
SET FILE1=\\%comp%\C$\Program Files (x86)\Google\Chrome\Application\new_chrome.exe
SET FILE2=%FILE1: (x86)=%
if exist "%FILE1%" (
	for /F "tokens=5" %%a in ('filever "%FILE1%"') do set File_Version=%%a
) Else (
...
SET ChromeTime=
IF "%File_Version%" NEQ "Not Found" (
	for /F "skip=3 tokens=8" %%a in ('pslist \\%comp% chrome') do CALL :CheckTime %%a
)
SET ChromeTime
SET OUTPUT=%Output%,%ChromeTime%
...
:CheckTime
IF "%ChromeTime%"=="" SET ChromeTime=%1
