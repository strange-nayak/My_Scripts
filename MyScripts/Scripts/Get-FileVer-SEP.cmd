...
for /D %%a in ("\\%COMP%\c$\Program Files\Symantec\Symantec Endpoint Protection\*") do IF EXIST "%%a\bin\ccSvcHst.exe" set File_Version=%%~na
IF "%File_Version%"=="Not Found" for /D %%a in ("\\%COMP%\c$\Program Files (x86)\Symantec\Symantec Endpoint Protection\*") do IF EXIST "%%a\bin\ccSvcHst.exe" set File_Version=%%~na

goto :skipit
SET FILE1=\\%comp%\C$\Program Files (x86)\Symantec\Symantec Endpoint Protection\12.1.4112.4156.105\Bin\ccSvcHst.exe
SET FILE2=%FILE1: (x86)=%
if exist "%FILE1%" (
