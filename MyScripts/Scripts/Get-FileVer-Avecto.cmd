...
@ECHO Machine,Accessible?,File Version,Arch>%ResultsFolder%\Header.txt
...
SET FILE1=\\%comp%\C$\Program Files\Avecto\Privilege Guard Client\DefendpointService.exe
if exist "%FILE1%" (
	for /F "tokens=5" %%a in ('filever "%FILE1%"') do set File_Version=%%a
) 
SET Arch=x86
If exist "\\%comp%\C$\Program Files (x86)" SET Arch=x64
SET OUTPUT=%Output%,%File_Version%,%arch%
